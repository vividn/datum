...
describe("addCmd", () => {
  ...
  it("stores the occurTime in the data", async () => {
    const newDoc = (await addCmd({date: "2021-08-23", time: "12", timezone: "0"})) as DatumDocument;
  ...
  it("stores the occurTime in DataOnly docs", async () => {
    const newDoc = (await addCmd({noMetadata: true, date: "2021-08-23", time: "12", timezone: "0"})) as DatumDocument;
  ...
  it("stores utcOffset using local timezone even if no occurTime is collected", async () => {
    ...
    const newDoc = (await addCmd({idPart: "unstamped", noTimestamp: true})) as DatumDocument;
