...
function animateTimeshift(targetTimeshift: number) {
  const svg = document.querySelector("svg");
  const dataArea = svg?.querySelector(".dataArea");
  if (!svg || !dataArea) return;
  ...
  dataArea.setAttribute("style", `transform: translateX(${pixelShift}px)`);

  // Update time labels
  updateTimeAxis(targetTimeshift);
}

function updateTimeAxis(newTimeshift: number) {
  const now = new Date();
...
function initializeKeyboardNavigation() {
  ...
  console.log(
    "Enhanced keyboard navigation initialized with timeshift: " +
      timeShiftMinutes,
  );
}
